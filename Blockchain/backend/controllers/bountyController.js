...
const NEXUSGUARD_ABI = [
  ...
  // ── Events (indexed string emits keccak hash; non-indexed copy carries the raw bugId) ──
...
async function handleMergeWebhook(req, res) {
  ...
  // Use the raw body captured by express.json()'s verify callback (see server.js).
  // JSON.stringify(req.body) would NOT reproduce the exact original bytes.
  ...
  // Detect native MATIC (address(0) = 18 decimals) vs ERC-20 (default 6 for USDC).
