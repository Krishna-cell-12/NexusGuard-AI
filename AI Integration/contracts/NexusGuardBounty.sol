...
contract NexusGuardBounty is Ownable, ReentrancyGuard {
     ...
     * @param bugId         Off-chain identifier (e.g., GitHub issue #).
     ...
     * @param bugId        Identifies the resolved vulnerability.
    ...
    function createBounty(
        ...
        emit VulnerabilityFound(bugId, msg.sender, tokenAddress, escrowed, block.timestamp);
    }
    ...
    function releaseBounty(
        ...
        emit BountyPaid(bugId, contributor, netPayout, b.tokenAddress, block.timestamp);
    }
    ...
    function cancelBounty(
        ...
        emit BountyCancelled(bugId, msg.sender, refund, block.timestamp);
    }
